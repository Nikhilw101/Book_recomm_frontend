...
function Bookhistory({ userId, onPreviewBook }) {
  ...
  const fetchUserHistory = async (userId) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/recommendations/history?user_id=${userId}`);
  ...
  const handleDeleteClick = async (bookId, bookTitle) => {
    ...
    if (isConfirmed) {
      try {
        await axios.delete(`http://localhost:5000/recommendations/history/${bookId}?user_id=${userId}`);
